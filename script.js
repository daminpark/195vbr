...
/**
 * Handles authentication by parsing a single URL parameter (?booking=...).
...
/**
 * Displays a generic "Access Denied" error message.
 */
function displayAuthError(guidebookContainer, tocContainer) {
  const errorHtml = `
    ...
    <section id="error-message">
      ...
      <p>The link you have used is not valid. Please use the exact link provided in your booking confirmation.</p>
    </section>
  `;
...
/**
 * Fetches configuration and builds the guidebook after successful authentication.
 * @param {string} bookingKey - The validated booking key for the guest.
 * @param {string} accessLevel - The access level ('full' or 'partial') from the API.
 */
async function buildGuidebook(bookingKey, accessLevel) {
  ...
  try {
    ...
    if (!bookingConfig) throw new Error(`Booking key "${bookingKey}" not found post-auth.`);
...
function createSmartHomeControlCards(container, bookingConfig) {
    let controlsHtml = `
        <div class="ha-card">
            ...
            <div class="ha-card-controls"><span>Controls Coming Soon</span></div>
        </div>
        <div class="ha-card">
...
// --- All other helper functions below this point are standard and remain unchanged ---
// (No further edits are needed from here down)
    ...
    if (key === 'weather') {
      try {
        const [currentState, hourlyForecast, dailyForecast] = await Promise.all([
            fetchHAData(entityId, house, 'state'), fetchHAData(entityId, house, 'hourly_forecast'), fetchHAData(entityId, house, 'daily_forecast')
        ]);
...
function buildChatbotContextFromPage() {
  ...
  if (mainContainer) {
    ...
    const systemPrompt = "You are 'Victoria', a friendly AI assistant for 195VBR. Base answers ONLY on the guidebook info provided. For other questions, use general knowledge. Be concise, use Markdown for links.";
...
function addInitialBotMessage() {
    ...
    chatBox.innerHTML = `<div class="message-bubble bot-message"><p>Welcome to 195VBR! I'm Victoria, your AI assistant. Ask me anything about the guesthouse or your London trip.</p></div>`;
}
...
async function sendMessage() {
    ...
    chatBox.insertAdjacentHTML('beforeend', `<div class="message-bubble user-message"><p>${userInput}</p></div><div class="timestamp">${getTimeStamp()}</div>`);
    chatHistory.push({ role: 'user', content: userInput });
    userInputField.value = '';
    userInputField.disabled = true;
    sendBtn.disabled = true;
    chatBox.scrollTop = chatBox.scrollHeight;
    chatBox.insertAdjacentHTML('beforeend', `<div class="message-bubble bot-message typing-indicator"><span></span><span></span><span></span></div>`);
    chatBox.scrollTop = chatBox.scrollHeight;
    ...
    try {
        ...
        chatBox.insertAdjacentHTML('beforeend', `<div class="timestamp">${getTimeStamp()}</div>`);
    } catch (error) {
        console.error('Fetch error:', error);
        typingIndicator.remove();
        chatBox.insertAdjacentHTML('beforeend', `<div class="message-bubble bot-message"><p>Sorry, I'm having trouble connecting. Please try again later.</p></div><div class="timestamp">${getTimeStamp()}</div>`);
    } finally {
