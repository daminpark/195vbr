...
function getStaticContent() {
  return {
    ...
    'heating': { title: 'Heating and Cooling', emoji: '🌡️', html: `<p>You can control the temperature in your room using the valve (TRV) on your radiator. For bookings with smart home controls, you can also adjust this from the dashboard on this page.</p><p><strong>Cooling:</strong> We do not have air conditioning. We recommend keeping the window and curtains closed during sunny days and opening them in the evening.</p>` },
    'lights-note': { title: "A Note on Light Controls", emoji: "🔌", html: `<p>Please note that these smart lights can only be controlled from this app when the physical light switch on the wall is in the 'ON' position. If a light is unresponsive, please check that its corresponding wall switch is turned on first.</p>` },
